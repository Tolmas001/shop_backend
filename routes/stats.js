...
// Get Geospatial Data (empty until the orders/users tables store coordinates)
router.get('/api/stats/geospatial', authenticateAdmin, async (req, res) => {
  try {
    // No geospatial columns exist in the schema yet; return empty so the
    // admin panel only displays real data once coordinates are available.
    res.json([]);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
